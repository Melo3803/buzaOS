...
void ProcessMousePacket(){
    if (!MousePacketReady) return;
        ...
        if (MousePacket[0] & PS2Leftbutton){
            if (!LeftMouseDown) { // Eğer daha önce basılı değilse
                GlobalAppManager->HandleClick();
            }
            LeftMouseDown = true;
        }
        else {
        ...
        if (MousePacket[0] & PS2Rightbutton){
            if (!RightMouseDown) { // Eğer daha önce basılı değilse
        ...
        GlobalAppManager->RunCurrentApp(); // Aktif uygulamanın Run() fonksiyonunu çağır biraz hatali ama olsun
...
void InitPS2Mouse(){
    ...
    outb(0x64, 0xA8); //auxiliary device - mouse aktif
